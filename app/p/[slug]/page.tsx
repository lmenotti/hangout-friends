...
import { getPollBySlug, loadPlanPageInitialData } from '@/lib/planPageData'
...
function isPlanExpired(poll: { archived_at: string | null; expires_at: string | null }): boolean {
...
export default async function PlanSlugPage(
  ...
  const initialData = await loadPlanPageInitialData(poll)

  return (
    <PollPageClient
      id={poll.id}
      autoFillAvailability={fill === '1'}
